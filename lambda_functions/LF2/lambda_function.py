...
def get_recommendations(request):
    ...
    headers = headers = {
        ...
        'Host': "search-restaurants-hjebambt3pukrd3q3ps7gxwoeu.us-east-1.es.amazonaws.com",
...
def message_template(request, msg_content):
    ...
    c_data_people = request['People']
    c_data_date = request['Date']
    c_data_time = request['Time']

    content = 'Hello! Here are my ' + c_data_cuisine + ' restaurant suggestions for ' + \
        c_data_people + ' people, on ' + c_data_date + ' at ' + c_data_time + '.\n\n'
...
def extract_request(message):
    ...
    request['People'] = message.get('People').get('StringValue')
    request['Time'] = message.get('Time').get('StringValue')
